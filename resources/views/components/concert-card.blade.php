    ...
    <div class="aspect-[4/3] bg-neutral-900">
        ...
        @else
            <div class="flex h-full items-end bg-[linear-gradient(135deg,#111,#3f2a20)] p-5 text-white">
                <div><p class="text-xs uppercase tracking-widest text-orange-200">{{ $concert->artist }}</p><h3 class="text-2xl font-black">{{ $concert->name }}</h3></div>
            </div>
        @endif
    </div>
    <div class="space-y-3 p-5">
        <div>
            <h3 class="text-xl font-black">{{ $concert->name }}</h3>
        ...
        <a href="{{ auth()->check() ? route('user.concerts.show', $concert) : route('concerts.show', $concert) }}" class="block rounded-full bg-neutral-950 px-4 py-3 text-center text-sm font-bold text-white">Detail / Beli Tiket</a>
    </div>
</div>
