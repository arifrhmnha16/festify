...
<section class="mx-auto max-w-7xl px-4 py-12">
    ...
    <div class="mt-8 grid gap-4 md:grid-cols-3">
        ...
        <div class="rounded-lg border bg-white p-5"><p>Gelang aktif</p><strong class="text-3xl">{{ $user->eTickets()->whereHas('wristband', fn ($query) => $query->where('wristband_status', 'aktif'))->count() }}</strong></div>
    </div>
    <div class="mt-10 flex flex-wrap gap-3"><a class="rounded-full bg-neutral-950 px-5 py-3 font-bold text-white" href="{{ route('user.concerts') }}">Cari Konser</a><a class="rounded-full bg-orange-700 px-5 py-3 font-bold text-white" href="{{ route('user.tickets') }}">Tiket Saya</a><a class="rounded-full border px-5 py-3 font-bold" href="{{ route('user.wristbands') }}">Gelang Saya</a><a class="rounded-full border px-5 py-3 font-bold" href="{{ route('user.orders') }}">Riwayat Pemesanan</a></div>
